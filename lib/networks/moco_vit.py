...
class MoCoViTEncoder(nn.Module):
...
class MoCoViTDecoder(nn.Module):
    """ Vision Transformer with support for patch or hybrid CNN input stage
    Modified from timm implementation
    """
...
def moco_encoder_small_patch16_224(**kwargs):
    model = MoCoViTEncoder(
        embed_dim=384,
        num_heads=6,
        **kwargs)
    return model

def moco_decoder_small_patch16_224(**kwargs):
    model = MoCoViTDecoder(
        embed_dim=128,
        depth=4,
        num_heads=3,
        **kwargs)
