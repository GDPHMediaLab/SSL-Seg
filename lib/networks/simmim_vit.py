...
class SimMIMViTEncoder(nn.Module):
...
class SimMIMViTDecoder(nn.Module):
    """ Vision Transformer with support for patch or hybrid CNN input stage
    Modified from timm implementation
    """
...
def simmim_encoder_small_patch16_224(**kwargs):
    model = SimMIMViTEncoder(
        embed_dim=384,
        num_heads=6,
        **kwargs)
    return model

def simim_decoder_small_patch16_224(**kwargs):
    model = SimMIMViTDecoder(
        embed_dim=128,
        depth=4,
        num_heads=3,
        **kwargs)
