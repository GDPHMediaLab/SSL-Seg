...
def sliding_window_inference(
    ...
    **kwargs: Any,
    ...
    buffered = buffer_steps is not None and buffer_steps > 0 #buffered:False
    # print("buffered:",buffered)
    num_spatial_dims = len(inputs.shape) - 2 #5-2=3
    overlap = ensure_tuple_rep(overlap, num_spatial_dims) # overlap: (0.5, 0.5, 0.5)
    for o in overlap:
        if o < 0 or o >= 1:
            raise ValueError(f"overlap must be >= 0 and < 1, got {overlap}.")
    compute_dtype = inputs.dtype # inputs.dtype: torch.float32
    batch_size, _, *image_size_ = inputs.shape
    device = device or inputs.device #device:0
