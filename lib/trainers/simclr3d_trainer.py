...
from lib.data.med_pretrain_transforms import get_simclr_pretrain_transforms, get_vis_transforms
...
class SimCLR3DTrainer(BaseTrainer):
    r"""
    SimCLR3D Trainer
    """
    ...
    def __init__(self, args):
        super().__init__(args)
        self.model_name = 'SimCLR_ViT'
        self.scaler = torch.cuda.amp.GradScaler()
        self.contrastive_loss = ContrastiveLoss(
            batch_size=args.batch_size * 2, temperature=args.temperature
        )
    ...
    def build_dataloader(self):
        if self.dataloader is None:
            ...
            args = self.args
            train_transform = get_simclr_pretrain_transforms(args)
    ...
    def run(self):
        ...
        niters = cur_epoch * self.iters_per_epoch
        print("start_epoch:", cur_epoch)
    ...
    def epoch_train(self, epoch, niters):
        ...
        for i, batch_data in enumerate(train_loader):
            ...
            outputs_v1 = self.model(image)
            outputs_v2 = self.model(image_1)

            flat_out_v1 = outputs_v1.flatten(start_dim=1)
            flat_out_v2 = outputs_v2.flatten(start_dim=1)

            # compute output and loss
            forward_start_time = time.time()
            with torch.cuda.amp.autocast(True):
                # loss = model(image, return_image=False)
                loss = self.contrastive_loss(flat_out_v1, flat_out_v2)
