...
from lib.data.med_pretrain_transforms import get_mocov3_pretrain_transforms, get_vis_transforms
...
class MoCo3DTrainer(BaseTrainer):
    ...
    def build_dataloader(self):
        if self.dataloader is None:
            ...
            args = self.args
            train_transform = get_mocov3_pretrain_transforms(args)
    ...
    def adjust_moco_momentum(self, epoch, args):
    ...
    def epoch_train(self, epoch, niters):
        ...
        for i, batch_data in enumerate(train_loader):
            ...
            moco_m = self.adjust_moco_momentum(epoch + i / self.iters_per_epoch, args)

            # compute output and loss
            forward_start_time = time.time()
            with torch.cuda.amp.autocast(True):
                loss = model(image, image_1, moco_m)
