...
class SimMIM3D(nn.Module):
    ...
    def __init__(self,
        ...
        self.mask_token_else = nn.Parameter(torch.randn(args.encoder_embed_dim))

        self.patch_norm = nn.LayerNorm(normalized_shape=(out_chans,), eps=1e-6, elementwise_affine=False)

        self.criterion = nn.MSELoss()

        self.to_pixels = nn.Linear(args.encoder_embed_dim, args.patch_dim)
    ...
    def forward(self, x, return_image=False):
        ...
        length = np.prod(self.grid_size)  # length=216
        sel_length = int(length * (1 - args.mask_ratio))  # sel_length = 54
        msk_length = length - sel_length  # msk_length = 162

        # prepare mask tokens
        mask_tokens = repeat(self.mask_token_else, "d -> b n d", b=batch_size, n=length)
        ...
        sel_x_pat_emb = self.encoder.patch_embed(sel_x)  # [B*L, embed_dim]
        sel_x_pat_emb = sel_x_pat_emb.reshape(B, L, embed_dim)

        all_x_pat_emb = torch.cat([sel_x_pat_emb, self.mask_token_else.expand(batch_size, msk_length, -1)], dim=1)

        shuffled_encoder_pos_embed = self.encoder_pos_embed.expand(batch_size, -1, -1).gather(dim=1,
                                                                                              index=shuffle_indices[:,
                                                                                                    :, None].expand(-1,
                                                                                                                    -1,
                                                                                                                    args.encoder_embed_dim))

        all_x_pat_emb = all_x_pat_emb + shuffled_encoder_pos_embed

        for blk in self.encoder.blocks:
            all_x_pat_emb = blk(all_x_pat_emb)

        all_x_pat_emb = self.encoder.norm(all_x_pat_emb)

        pred_pixel_values = self.to_pixels(all_x_pat_emb)

        loss = self.criterion(input=pred_pixel_values[:, -msk_length:, :], target=self.patch_norm(msk_x.detach()))
        ...
        if return_image:
            ...
            recon = pred_pixel_values[:, :, :].gather(dim=1,
